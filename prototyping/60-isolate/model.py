...
class S1Transformer(L.LightningModule):
    ...
    def __init__(self, d_model, n_enc_heads, n_enc_layers, n_tokens, lr, weight_decay, pad_token_id, predictive, dropout, **kwargs):
        ...
        self.enc = nn.TransformerEncoder(mb.TransformerEncoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
        self.dec = nn.TransformerDecoder(mb.TransformerDecoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
        ...
        if self.predictive:
            self.pred_enc = nn.TransformerEncoder(mb.TransformerEncoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
    ...
    def model_step(self, batch, batch_idx, mode='train'):
        ...
        x, y, sl = batch # (bs, seq_len), y is (bs,)
        padding_mask = (x==self.pad_token_id) # (bs, seq_len)
        x, y, sl = x.to(torch.long), y.to(torch.long), sl.to(torch.long) # do conversion on GPU (mem bottleneck)
        ...
        if self.predictive:
            ...
            # for code, what this is doing is testing the encodings which end
            # in a close-) and having the system learn to predict intermediates


            seq_len, bs, d_model = x_enc.shape
            x_enc_long = x_enc.reshape(1, -1, d_model) # (1, seq_len*bs, d_model)
            pad_long = padding_mask.reshape(-1, 1) # (bs*seq_len, 1)

            # make prediction
            y_pred_enc_long = self.pred_enc(x_enc_long) # (1, seq_len*bs, d_model)
            y_pred = self.pred_linear(y_pred_enc_long) # (1, seq_len*bs, n_tokens)
            ...
            loss_pred = F.cross_entropy(ce_y_pred, sl, ignore_index=self.pad_token_id)
            self.log(f"{mode}_pred_loss", loss_pred)
            # accuracy
            y_hat = torch.argmax(y_pred, dim=2) # (bs, seq_len)
            self.metrics[f'{mode}_pred_acc'](y_hat, sl)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.RMSprop(self.parameters(), lr=self.lr, weight_decay=self.wd, alpha=0.99, eps=1e-8)
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
