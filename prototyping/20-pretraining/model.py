...
class System1(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        mask = torch.nn.Transformer.generate_square_subsequent_mask(sz=x.shape[0], device=x.device) # (seq_len, seq_len)
        s1 = self.transformer_encoder(x, mask=mask, src_key_padding_mask=padding_mask, is_causal=True) # (seq_len, bs, d_model)
...
class S1OutputTranslator(nn.Module):
    ...
    def forward(self, s1_proposed, padding_mask):
        ...
        output_pred = self.linear(s1_proposed[-padding_mask.shape[1]:, :, :]) # (seq_len, bs, n_output_tokens)
...
class BPTTTransformer(L.LightningModule):
    ...
    def forward(self, x: torch.Tensor, padding_mask: torch.Tensor=None, n_steps: int=1) -> torch.Tensor:
        ...
        for i in range(n_steps):
            # run s1
            s1_proposed = self.s1(x, padding_mask) # (2*seq_len +1, bs, d_model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x, yi, yf, padding_mask = batch # (bs, seq_len)
        ...
        loss = F.cross_entropy(ce_y_hat, yi, ignore_index=self.pad_token_id)
        
        self.log("train_loss", loss)

        # accuracy
        y_pred = torch.argmax(y_hat, dim=2).T # (bs, seq_len)
        self.train_acc(y_pred, yi)
