...
class System1(nn.Module):
    ...
    def forward(self, x_orig, x_raw, padding_mask):
        """
...
class BPTTTransformer(L.LightningModule):
    ...
    def forward(self, o: torch.Tensor, x: torch.Tensor, padding_mask: torch.Tensor=None, n_steps: int=1) -> torch.Tensor:
        """
        x: (bs, seq_len)
        """
        ...
        for i in range(n_steps):
            # run s1
            s1_proposed = self.s1(o, x, padding_mask) # (2*seq_len +1, bs, d_model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        o, x, y, padding_mask = batch # (bs, seq_len)
        ...
        y_hat = self(o, x, padding_mask=padding_mask, n_steps=1) # (seq_len, bs, n_output_tokens)
