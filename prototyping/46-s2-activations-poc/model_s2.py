...
class S2Model(nn.Module):
    ...
    def __init__(self, n_bptt, d_model, n_enc_heads, n_enc_layers, n_tokens, weight_decay, pad_token_id, dropout):
        ...
        self.enc_x_s2 = nn.TransformerEncoder(mb.TransformerEncoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
        self.dec_s2_1 = nn.TransformerDecoder(mb.TransformerDecoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
        self.dec_s2_2 = nn.TransformerDecoder(mb.TransformerDecoderLayer(d_model=d_model, nhead=n_enc_heads, activation=F.leaky_relu, dropout=self.dropout, batch_first=False), num_layers=n_enc_layers)
    ...
    def forward(self, x_enc: torch.Tensor, s1_model) -> torch.Tensor:
        ...
        # prep output tensor
        s2_pred = torch.zeros(self.n_bptt, 1, bs_sl, self.n_tokens, device=x_enc.device) # (n_bptt, 1, bs_sl, n_tokens)
