...
class System1(nn.Module):
    ...
    def forward(self, x_raw, wm, padding_mask):
        ...
        enc_in = torch.cat([wm, x], dim=0) # (seq_len*2, bs, d_model)
        enc_in = self.pos_encoder(enc_in) # (seq_len*2, bs, d_model)

        # double up padding mask
        padding_mask = torch.cat([padding_mask, padding_mask], dim=1) # (bs, seq_len*2)

        # push through transformer
        s1_proposed = self.transformer_encoder(enc_in, src_key_padding_mask=padding_mask) # (seq_len*2, bs, d_model)
...
class S1OutputTranslator(nn.Module):
    ...
    def forward(self, s1_proposed, padding_mask):
        ...
        output_pred = self.linear(enc_out_x) # (seq_len, bs, n_output_tokens)
...
class BPTTTransformer(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat = self(x, padding_mask=padding_mask, n_steps=n_steps) # (seq_len, bs, n_output_tokens)
