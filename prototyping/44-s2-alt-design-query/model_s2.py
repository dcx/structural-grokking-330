...
class S2Model(nn.Module):
    ...
    def forward(self, x_enc: torch.Tensor, s1_enc: torch.Tensor, s1_model) -> torch.Tensor:
        """
        ...
        s1_resp = s1_model.pred_enc(s1_query) # (1, bs*seq_len, d_model)
        # push through decoder: update state
        s2_phase1 = self.dec_s2_1(s2_state, s1_resp) # (2, bs*seq_len, d_model)
        ...
        # simple linear prediction over s2_state's x_enc
        y_pred = self.linear_out(s2_phase1[1:]) # (1, bs*seq_len, n_tokens)
...
class S2Transformer(L.LightningModule):
    ...
    def model_step(self, batch, batch_idx, mode='train'):
        ...
        # Predict using S2
        s2_y_pred = self.s2_model(lc_x_enc, lc_y_pred_enc, self.s1_model) # (n_bptt, 1, bs_s2, n_tokens)
